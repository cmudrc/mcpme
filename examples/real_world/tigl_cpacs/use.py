"""Real-world example for ingesting, serving, and then using a TiGL CPACS helper.
...
This real-world example requires the real `tigl3` and `tixi3` Python bindings, which
are typically installed outside the base Python toolchain. The repository does
not install them automatically, so the real-world example is expected to skip cleanly
on many machines.
...
- ``examples/real_world/tigl_cpacs/ingest.py``
- ``examples/real_world/tigl_cpacs/serve.py``
- ``examples/support/real_world/tigl_cpacs/commands/scaffold_tigl_cpacs.sh``
- ``examples/support/real_world/tigl_cpacs/tigl_support/__init__.py``
- ``examples/support/real_world/tigl_cpacs/tigl_support/core.py``
- ``examples/support/real_world/tigl_cpacs/fixtures/CPACS_30_D150.xml``
- ``docs/quickstart.rst``
"""
...
def main() -> None:
    ...
    # Spawn the MCP server as a separate process so the real-world example exercises
    ...
    if server.stdin is None or server.stdout is None or server.stderr is None:
        raise RuntimeError(
            "Expected stdio pipes when launching the TiGL real-world example MCP server."
        )
    ...
    try:
        ...
        try:
            ...
            # If the server hangs, kill it so the real-world example still terminates
