...
This example shows the simplest useful `mcpcraft` workflow: expose a typed Python
...
- Import only the curated top-level public API from `mcpcraft`.
- Define a typed callable with a Sphinx-style docstring and an `MCP:` block.
- Build a :class:`mcpcraft.Manifest` directly from that callable.
...
from mcpcraft import Manifest, build_manifest
