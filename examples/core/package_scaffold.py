...
Python facade first, then wrapped through the normal `mcpcraft` manifest flow.
...
`artifacts/examples/core/package_scaffold/`.
...
- Build a manifest from the generated facade and execute both function and
  class-session tools through :func:`mcpcraft.execute_tool`.
...
facade remains on disk under `artifacts/examples/core/package_scaffold/`.
...
ARTIFACT_ROOT = REPO_ROOT / "artifacts" / "examples" / "core" / "package_scaffold"
