...
def _resolve_config_table(data: dict[str, Any]) -> dict[str, Any]:
    """Return the ``tool.mcpcraft`` table from parsed TOML data."""
    if "tool" in data and isinstance(data["tool"], dict) and "mcpcraft" in data["tool"]:
        table = data["tool"]["mcpcraft"]
        if isinstance(table, dict):
            return table
    if "mcpcraft" in data and isinstance(data["mcpcraft"], dict):
        return data["mcpcraft"]
    return {}
...
def _load_toml_path(config_path: Path | None) -> tuple[Path | None, dict[str, Any]]:
    ...
    candidate_paths = (
        ...
        else (Path("pyproject.toml"), Path("mcpcraft.toml"))
    )
...
def load_config(config_path: str | Path | None = None) -> ManifestConfig:
    ...
    root_dir = Path(table.get("artifact_root", ".mcpcraft-artifacts"))
