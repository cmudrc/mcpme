...
def _ensure_avl(install_root: Path, force: bool) -> InstalledTool:
    ...
    with tempfile.TemporaryDirectory(prefix="mcpcraft-avl-") as temp_dir:
...
def _ensure_xfoil(install_root: Path, force: bool) -> InstalledTool:
    ...
    with tempfile.TemporaryDirectory(prefix="mcpcraft-xfoil-") as temp_dir:
        temp_path = Path(temp_dir)
...
def _ensure_su2(install_root: Path, force: bool) -> InstalledTool:
    ...
    with tempfile.TemporaryDirectory(prefix="mcpcraft-su2-") as temp_dir:
        temp_path = Path(temp_dir)
