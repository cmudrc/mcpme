...
`mcpcraft` keeps the supported top-level API intentionally small. Rather than
...
- ``mcpcraft scaffold-package`` for installed Python packages or modules
- ``mcpcraft scaffold-command`` for standalone CLI tools
- ``mcpcraft scaffold-openapi`` for OpenAPI-described HTTP surfaces
