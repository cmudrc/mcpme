...
from mcpwrap import ArgparseCommand, build_manifest, execute_tool
from mcpwrap.cli import main
...
def test_module_invocation_runs_cli_main(tmp_path: Path) -> None:
    """`python -m mcpwrap.cli` should execute the CLI entry point."""
    source = tmp_path / "mesh_tools.py"
    source.write_text(
        """def mesh_model(input_path: str) -> str:\n"""
        '''    """Generate a mesh.\n\n'''
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "mcpwrap.cli", "manifest", str(source)],
        capture_output=True,
        text=True,
        env=env,
        check=False,
