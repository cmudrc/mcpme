...
PACKAGE_INIT = REPO_ROOT / "src" / "mcpwrap" / "__init__.py"
...
def _collect_package_aliases(module: ast.Module) -> set[str]:
    """Collect local aliases used for ``import mcpwrap`` statements."""
    aliases: set[str] = set()
    for node in ast.walk(module):
        if not isinstance(node, ast.Import):
            continue
        for alias in node.names:
            if alias.name == "mcpwrap":
                aliases.add(alias.asname or "mcpwrap")
    return aliases
...
def _explicit_imported_symbols(module: ast.Module, exports: set[str]) -> set[str]:
    ...
    covered: set[str] = set()
    for node in ast.walk(module):
        if not isinstance(node, ast.ImportFrom) or node.module != "mcpwrap":
            continue
        for alias in node.names:
            if alias.name == "*":
...
def _attribute_symbols(module: ast.Module, exports: set[str], aliases: set[str]) -> set[str]:
    """Collect public symbol usage through ``mcpwrap.<symbol>`` access."""
    covered: set[str] = set()
    for node in ast.walk(module):
